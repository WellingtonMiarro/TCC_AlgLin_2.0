class Gauss {
  gauss_dois() {
    console.log("ATENÇÃO! Somente para ordem 2 como exemplo");
    console.log("\n\n");
    ...
    for (
      i = 0;
      i < 2;
      i++ //i < 2 na generalização para qualquer ordem seria algo como lenght ou size da matriz, que tem q retornar a ordem dela. Será fácil pegar esse valor a partir da entrada do usuário no início
    ) {
      for (j = 0; j < 2; j++) {
    ...
    if (opt == 1) {
      ...
      let matrizcompletainv = [[], []]; // 4 aqui quer dizer "o dobro da ordem da matriz"
      for (
        i = 0;
        i < 2;
      ...
      for (i = 0; i < 2; i++) {
        for (j = 0; j < 4; j++) {
          console.log("[", matrizcompletainv[i][j], "]");
        }
        console.log("\n");
      }
